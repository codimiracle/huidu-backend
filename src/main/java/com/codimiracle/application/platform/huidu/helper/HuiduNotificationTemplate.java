...
public class HuiduNotificationTemplate implements NotificationTemplate {
    ...
    @Override
    public Notification generateSubscribeBy(SubscribeType type, String receiverId, String targetId) {
    ...
    @Override
    public Notification generateNotificationBy(SubscribeType type, String receiverId, String targetId, String extraMessage) {
        if (type == SubscribeType.BookUpdated) {
            BookVO bookVO = bookService.findByIdIntegrally(targetId);
            Notification notification = new Notification();
            notification.setReceiverId(receiverId);
            notification.setType(NotificationType.Subscribe);
            String name = Objects.isNull(bookVO.getTitle()) ? bookVO.getMetadata().getName() : bookVO.getTitle();
            notification.setMessage(String.format("%s 图书更新了：%s", name, extraMessage));
            return notification;
        }
        return null;
    }
    ...
    public Notification generateCancelBy(Subscribe subscribe) {
        if (subscribe.getType() == SubscribeType.BookUpdated) {
            ...
            if (Objects.isNull(bookVO)) {
                ...
                name = Objects.isNull(bookVO.getTitle()) ? bookVO.getMetadata().getName() : bookVO.getTitle();
            }
